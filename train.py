...
def get_learning_rate(epoch, sub_num, rand_num):
    ...
    lc = tf.compat.v1.train.exponential_decay(learning_rate=BASE_LEARNING_RATE, decay_rate=0.5,
...
def train():
    with tf.Graph().as_default():
        with tf.device('/device:gpu:0'):
            ...
            point_clouds, label_points = model.placeholder_inputs(BATCH_SIZE, NUM_POINT)
            ...
            loss = model.get_loss(s_id, s_exp, s_pred, label_points, end_points, LAMBDA1, LAMBDA2)
            tf.compat.v1.summary.scalar('loss', loss)

            epoch_lc = tf.compat.v1.Variable(1)
            learning_rate = get_learning_rate(epoch_lc, 1500, 10)
            tf.compat.v1.summary.scalar('learning_rate', learning_rate)
            optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate)
            train_op_adam = optimizer.minimize(loss, global_step=epoch_lc)
        ...
        for epoch in range(1, MAX_EPOCH+1):
            ...
            # print(epoch_lc.eval(session=sess))

            is_training = True

            # 1500 subjects with 10 neutral expressions
            fn_len = len(os.listdir('./points_sampling'))
            ...
            for i in range(subject_num):
                for j in range(10):
                    ...
                    points_data = data_preprosessing.load_npyfile('./points_sampling/sub{0}_rand{1}.npy'.format(i, j))
            ...
            # np.save('./sub0_rand9_epoch{}'.format(epoch), s_id.reshape(29495, 3))
