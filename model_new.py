...
def variable_on_gpu(name, shape, initializer, use_fp16=False):
...
def variable_with_weight_decay(name, shape, stddev, wd, use_xavier=True):
    ...
    var = variable_on_gpu(name, shape, initializer)
    if wd is not None:
...
def conv2d(inputs, output_channels, kernel_size, scope,
    ...
    with tf.compat.v1.variable_scope(scope) as sc:
        ...
        biases = variable_on_gpu('biases', [output_channels], tf.constant_initializer(0.0))
...
def full_connected(inputs, num_outputs, scope, use_xavier=True, stddev=1e-3,
                   weight_decay=0.0, activation_fn='ReLU'):
    with tf.compat.v1.variable_scope(scope) as sc:
        ...
        biases = variable_on_gpu('biases', [num_outputs], tf.constant_initializer(0.0))
...
def get_model_encoder(point_cloud, is_training, bn_decay=None, bn=True):
...
def get_model_decoder(f_id, f_exp, num_point, end_points):

    d_id_net = full_connected(f_id, 1024, scope='fc_de_id')

    d_exp_net = full_connected(f_exp, 1024, scope='fc_de_exp')

    s_id = full_connected(d_id_net, num_point * 3, scope='fc_shape_id', activation_fn='Tanh')

    s_exp = full_connected(d_exp_net, num_point * 3, scope='fc_shape_exp', activation_fn='Tanh')
